...
from admin.schemas import SRequestCreate, SRequestUpdate, SUserCreate
...
@router.post("/add", dependencies = [Depends(get_current_superuser)], status_code=status.HTTP_201_CREATED)
async def add_user(
...
@router.delete("/delete/{user_id}", dependencies = [Depends(get_current_superuser)])
async def delete_user(user_id: str, session: AsyncSession = Depends(get_session)):
    await AdminService(session).delete(user_id)
    return JSONResponse({"message": "User was deleted successfully!"})


@router.put("/update", dependencies = [Depends(get_current_superuser)])
async def update_user(user: SUserCreate, session: AsyncSession = Depends(get_session)):
...
@router.get("/request", dependencies = [Depends(get_current_superuser)])
...
async def get_all_request(
    request: SRequestCreate,
    session: AsyncSession = Depends(get_session),
) -> RequestModel:
    return await AdminService(session).add_request(request)

@router.put("/request/update")
async def update_request(request: SRequestUpdate, session: AsyncSession = Depends(get_session)) -> RequestModel:
    return await AdminService(session).update(request.model_dump)

@router.get("/id/{user_id}", dependencies = [Depends(get_current_superuser)])
...
@router.get("/login/{login}", dependencies = [Depends(get_current_superuser)])
